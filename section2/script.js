...
function generateFactList(initialFacts) {
  const factArray = initialFacts.map(
    (localFacts) =>
      `<li class="factlist">
     <div class="factlistchild1">
      <p>${localFacts.text}</p>
      <span>${localFacts.category}</span>
    </div>
    <div class="voteBtn">
      <button>👍${localFacts.votesInteresting}</button>
      <button>🤯${localFacts.votesMindblowing}</button>
      <button>⛔${localFacts.votesFalse}</button>
    </div>
  </li>`
  );
